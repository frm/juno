defmodule Juno.ConfigTest do
  ...
  describe "config/3" do
    test "returns the default value if there is no application env variable" do
      Application.put_env(:juno, __MODULE__, test_value: 2)
    ...
    test "returns the correct application env variable if set" do
      Application.put_env(:juno, __MODULE__, test_value: 1)

      assert 1 == Config.config(__MODULE__, :test_value, nil)
    end

    test """
    uses the system env if the application env variable is in the {:system, var} format
    """ do
      Application.put_env(:juno, __MODULE__, test_value: {:system, "HOME"})

      assert System.get_env("HOME") ==
               Config.config(__MODULE__, :test_value, nil)
    end
  end

  describe "config!/2" do
    test "raises an error if there is no application env variable" do
      Application.put_env(:juno, __MODULE__,
